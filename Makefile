...
load-met-office-data:
	...
	JOIN_VIEW=iris.uk_ward DATA_VIEW=iris.uk_ward_epc_data MATERIALIZED_VIEW=iris.uk_ward_epc TARGET_TABLE=iris.district_borough_unitary_ward GPKG_SOURCE=https://api.os.uk/downloads/v1/products/BoundaryLine/downloads?area=GB&format=GeoPackage&redirect GPKG_TABLE=district_borough_unitary_ward python developer-resources/load_gpkg_to_postgis.py
	JOIN_VIEW=iris.uk_ward DATA_VIEW=iris.uk_ward_epc_data MATERIALIZED_VIEW=iris.uk_ward_epc TARGET_TABLE=iris.unitary_electoral_division GPKG_SOURCE=https://api.os.uk/downloads/v1/products/BoundaryLine/downloads?area=GB&format=GeoPackage&redirect GPKG_TABLE=unitary_electoral_division python developer-resources/load_gpkg_to_postgis.py
